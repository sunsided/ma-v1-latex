...
Diese Vorgehensweise führt in diesem Beispiel folglich zu einer Verdoppelung der nutzbaren Datenmenge gegenüber des naiven Ansatzes.
...
\todo[inline]{1st bis 4th gen \url{http://de.slideshare.net/sbaltagi/overview-of-apacheflinkbyslimbaltagi/12?src=clipshare}}

Hadoop ist eine Implementierung des \algo{MapReduce}-Algorithmus zur verteilten Datenverarbeitung;
Verarbeitungen werden über die beiden Funktionen \algo{map} (Transformation der Eingänge) und \algo{reduce} (Aggregation der Zwischenergebnisse) abgebildet.
Aus dem Big Data-Umfeld stammend ist Hadoop bietet konsequent eine spezialisierte Platform für die Batch-Verarbeitung extrem großer Datenmengen.
Der YARN genannte Ressourcenmanager verteilt die Arbeitslast auf die vorhandenen Arbeitinstanzen.
Die Lebenszeit eines Hadoop-Prozesses ist endlich, 
Prozesse werden immer explizit gestartet und terminieren nach der Verarbeitung des Datensatzes.
Der Datenaustausch zwischen Mappern und Reducern, 
sowie dem Initiator der Verarbeitung erfolgt ausschließlich über das HDFS-Clusterdateisystem, 
wodurch Hadoop für sehr schnellebige Prozesse ungeeignet wird.

Apache Spark baut auf den Prinzipien von Hadoop auf, spezialisiert sich aber auf die In-Memory-Verarbeitung von Daten und erreicht somit eine höhere Verarbeitungsgeschwindigkeit.
Ein Spark Streaming genanntes Interface ist in der Lage,
Datenströme auf kleine Arbeitspakete aufzuteilen (Micro-Batching) und somit Streamverarbeitung nachzubilden.
\todo[inline]{Spark ungeeignet für Echtzeit-Anfragen}

Apache Storm ist eine Lösung zur Echtzeit-Streamingverarbeitung von Ereignissen.
Storm definiert die Datenverarbeitung in Form von \algo{spouts} (Datenquellen) und \algo{bolts} (Datenprozessoren) als gerichteten, azyklischen Graphen.
Eine Trident genannte High Level-API sammelt die eingehenden Daten zu Batches (Micro-Batching analog Spark Streaming) und erlaubt die Komposition von MapReduce-artigen Verarbeitungsgraphen.
Spark verwendet für das Clustermanagement Apache ZooKeeper und funktioniert
prinzipiell zustandsfrei, wodurch das Erholen von Systemausfällen trivial ist.
Für zustandsbehaftete Verarbeitung können Transaktionen und States definiert werden.
Die DRPC genannte Schnittstelle für (distributed) remote procedure calls erlaubt das Abfragen des internen States oder das Starten eines Verarbeitungsgraphen "on demand".

\todo[inline]{Apache Flink, \url{http://stackoverflow.com/questions/28082581/what-is-the-differences-between-apache-spark-and-apache-flink}}
\todo[inline]{Zyklische und iterative Datenflüsse}
Apache Flink bietet Unterstützung für Batch und echte Streamingverarbeitung (d.h. kein Micro-Batching) und erlaubt die Definition von zyklischen Graphen,
sowie iterativen Prozessen.
Flink kann sowohl standalone, als auch mittels des YARN-Ressourcenmanagers betrieben werden; für high availability-Zwecke wird in der standalone-Umgebung Apache ZooKeeper verwendet.
...
\todo[inline]{Veraltet - legacy-Treiber wurde letztlich verwendet}

\section{OpenCV}

Für alle Builds kamen CMake 3.4 und Visual Studio 2015  unter Windows 10 zum Einsatz,
wobei für die Intel x64-Architektur kompiliert wurde.
Die Features AVX, AVX3, FMA3, SSE, SSE2, SSE3, SSSE3, SSE4.1, SSE4.2, sowie die automatisch bereitgestellten Intel Integrated Performance Primitives ("`ippicv"') 9.0.1 und OpenMP wurden aktiviert.
Debug- und Releasebuilds wurden wie üblich erstellt.

\subsection{Intel Threading Building Blocks}

TBB 4.4 Update 2 wurde als vorkompilierte Binary von den offiziellen Quellen bezogen (\url{https://www.threadingbuildingblocks.org/}).
Die Verwendung in OpenCV wurde aktiviert (\texttt{WITH\_TBB}), führte jedoch zu keiner signifikanten Verbesserung der Performance gegenüber den bereits vorhandenen Alternative OpenMP.
