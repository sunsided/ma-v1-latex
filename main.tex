...
\newcommand{\LAB}{L\textsuperscript{\tiny *}a\textsuperscript{\tiny *}b\textsuperscript{\tiny *}\index{Lab}}
\newcommand{\LUV}{L\textsuperscript{\tiny *}u\textsuperscript{\tiny *}v\textsuperscript{\tiny *}\index{Luv}}
\newcommand{\RGB}{RGB\index{RGB}}
\newcommand{\HLS}{HLS\index{HLS}}
\newcommand{\HSV}{HSV\index{HSV}}

\newcommand{\MATLAB}{MATLAB\index{MATLAB}}
...
\Cite{Witkin:1983:SF:1623516.1623607,1172729} definiert den Skalenraum (engl. \textit{Scale-Space}) eines Signals $f(z)$ als die Menge der Faltungen des Signals mit einem anisotropen Skalierungsoperator $g(z,\sigma)$ über alle Skalierung $\sigma$ als
%
\begin{align}
\phi(x,y,\sigma) &= \fun f(x,y,\sigma) \ast \fun g(x,y,\sigma) \label{eq:scale-space}
\end{align}
%
Er wählt für diesen Operator die Gaußfunktion\footnote{Der Gaußoperator ist die Wahrscheinlichkeitsdichtefunktion der Normalverteilung.}
%
\begin{align}
\fun g(x,y,\sigma) &= \frac{1}{2\pi\sigma_x\sigma_y} \myexp{-\frac{(x-\mu_x)^2}{2 \sigma_x^2} - \frac{(y-\mu_y)^2}{2 \sigma_y^2}}
\end{align}
%
bzw. für mittelwertfreie Signale mit einheitlicher Standardabweichung (bzw. Skale) in den räumlichen Dimensionen
%
\begin{align}
\fun g(x,y,\sigma) &= \frac{1}{\sigma\sqrt{2\pi}} \myexp{-\frac{x^2 + y^2}{2 \sigma^2}} \label{eq:gaussian_2d}
\end{align}
%
und zeigt mit \cite{4767749}, dass dieser der einzige für die Ermittlung des Skalenraums in Frage kommende Operator ist, so dass mit \cref{eq:scale-space,eq:gaussian_2d} gilt
%
\begin{align}
\mat \Phi[x,y,\sigma] &= \mat I[x,y] \ast \mat G[x,y,\sigma]
\end{align}
%
...
\section{Laplacian of Gaussian und Difference of Gaussian}



\todo[inline]{auch über LoG und DoG schreiben ...}

\todo[inline]{\cref{fig:sift_scales} referenzieren}

Der Einfluss von Werten außerhalb $\mu \pm 3\sigma$ kann für die Betrachtung des Gaußoperators vernachlässigt werden (vgl. \cite[176]{sonka2008image}); 
um jedoch einen zu starken Beschnitt der Werte in den Randbereichen des Kernes zu vermeiden, 
wählt etwa \cite{Nguyen:2007:GG:1407436} einen entsprechenden Filterradius ("`support"') von $\floor*{3\sigma}$ je Dimension.
Die grundsätzliche Abhängigkeit der Filtergröße von seiner Parametrierung führt jedoch (ungeachtet der Ausnutzung der Separierbarkeit) im vorliegenden Fall zu einem ebenfalls exponential steigenden Rechenaufwand, 
weswegen \cite{lowe1999recognition} einen Kern der festen Breite $7$ (d.h. einen Radius von drei Werten in jeder Dimension) verwendet.
...
\Cite{lowe2004distinctive} definiert $k$ als eine Funktion der Anzahl der benötigten Skalen $s$ als
%
\begin{align}
k &= 2^{\sfrac{1}{s}}
\end{align}
...
\Citeauthor{lowe2004distinctive} geht davon aus, dass jedes Originalbild $I$ bereits als mit $\sigma_n=0.5$ geglättet angenommen werden kann\todo{aber wieso?}, weswegen das Erzeugen des Skalenraumes anstelle von
%
\begin{align}
