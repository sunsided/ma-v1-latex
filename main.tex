...
\subsection{Normalisierung des Wertebereiches}
...
Berechnung des Mittelwertes eines Bildes $\mat I$ mit $N$ Pixeln ergibt sich aus
%
\begin{align}
\mean{\mat I} &= \frac{1}{N} \sum_x \sum_y \mat I[x, y]
\end{align}
...
Hierbei muss beachtet werden, dass kein Überlauf der Summenvariablen stattfindet; dass dies unwahrscheinlich ist, zeigt folgende Betrachtung:
Bei einem Bild mit 8 Bit Farbtiefe pro Kanal je Pixel (Wertebereich $0 \dots 255$) entspricht die maximal erreichbare Summe $N \times 255$.
Bei einem Bild mit \SI{4}{\mega\px} (z.B. $\si{2000} \times \SI{2000}{\px}$) entspricht dies dem Wert $\si{4000000} \times 255 = \si{1020000000}$, d.h. 1,2 Milliarden.
Ein vorzeichenfreier 32-Bit-Datentyp bildet den Wertebereich $2^0 \dots 2^{32} = 0 \dots \si{4294967295}$ ab, das heißt, die maximale kumulative Intensität beträgt ca. 4,3 Milliarden.
...
\subsection{Energiebasiertes Retargeting mittels Sobel-Operator}
...
\subsubsection{Eindimensionale Faltung zur approximativen Differenzierung}

Eine Approximation der Ableitung $\sfrac{\partial \fun f}{\partial x}$ ist der zentrale Differenzenquotient
%
\begin{align}
\frac{\partial \fun f}{\partial x} &\approx \frac{\fun f(x+\Delta x) - \fun f(x-\Delta x)}{2\Delta x} \label{eq:centralDifferenceOperator}
\end{align}
%
...
\subsubsection{Sobel-Operator und zweidimensionale Faltung}
...
%
\begin{align}
\fun e_1(\mat I) &= |\mat G_x \ast \mat I| + |\mat G_y \ast \mat I| \label{eq:sobelEnergy}
\end{align}
%
...
\subsubsection{Separierbare Faltungskerne}
...
\begin{figure}
    ...
    \caption*{Nur der zentrale Bildausschnitt wird dargestellt. Eine Skalierung um $\sfrac{1}{8}$ und Offsetverschiebung um $+127$ zurück in den Wertebereich $0 \dots 255$ ($0$ entspricht schwarz, $255$ entspricht weiß) wurde für Darstellungszwecke vorgenommen; Neutralgrau (Helligkeitswert $127$) entspricht Kantenfreiheit, dunklere Töne einer steigenden Kante (Suchrichtung links nach rechts bzw. oben nach unten), hellere Töne einer fallenden Kante.}
\end{figure}
