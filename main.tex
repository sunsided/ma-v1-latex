...
Eine Approximation der Ableitung $\dfrac{\partial}{\partial x}$ ist der Rückwärtsdifferenzenquotient
%
\begin{align}
\frac{\partial f}{\partial x} &\approx \frac{f(x) - f(x - \Delta x)}{\Delta x} \label{eq:differenceOperator}
\end{align}
%
welcher unter der Annahme einer diskreten Schrittweite $\Delta x = 1$ über den folgenden Faltungskernel abgebildet werden kann:
%
\begin{align}
\Delta f &= \begin{bmatrix}
 1 & -1
 \end{bmatrix} \ast f \label{eq:len2diffKernel}
\end{align}
%
Eine Anwendung des in \cref{eq:len2diffKernel} dargestellten Differenzenkernels ist in \cref{lst:len2diffKernel} zu finden.

Aus der Kombination von Vorwärts- und Rückwärtsdifferenzenquotient (\cref{eq:differenceOperator}) um einen zentralen Wert $x$ ergibt sich der zentrale Differenzenquotient:
%
\begin{align}
\frac{\partial f}{\partial x} &\approx \frac{f(x+\Delta x) - f(x-\Delta x)}{2\Delta x} \label{eq:centralDifferenceOperator}
\end{align}
%
der sich, wie zuvor, als Faltungskernel um einen zentralen Wert darstellen lässt:
%
\begin{align}
\Delta f = \begin{bmatrix}
 1 & 0 & -1
 \end{bmatrix} \ast f \label{eq:len3diffKernel}
\end{align}
%
