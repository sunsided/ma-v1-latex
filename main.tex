...
Die Berechnung der Skalenbilder wird unterteilt in mehrere Oktaven $O$, sowie einer Anzahl von Skalen $S$ pro Oktave.
Ausgehend von einer Basisglättung $\sigma_0$ (Skale 0) wird nun für alle $s=1,2,\dots,S$ je Oktave das Ursprungsbild $I$ mit dem Vielfachen $k^s \sigma_0$ geglättet, 
wobei $k$ eine noch festzulegende Konstante darstellt (\cref{eq:k_by_s}).
Ein Vorteil der Unterteilung der Berechnung in Oktaven ergibt sich aus dem Umstand, 
dass die relative Genauigkeit eines mit $3.5\sigma$ gefilterten Bildes sich nicht wesentlich von der Genauigkeit einer mit $2\sigma$ gefilterten in der Größe halbierten Version des mit $2\sigma$ gefilterten Bildes unterscheidet (\cite{lowe2004distinctive}, vgl. \cite{shah:online:2012});
durch die Reduktion der Bildfläche um Faktor vier wird der erforderliche Rechenaufwand mit jeder Oktave drastisch reduziert. \Cref{fig:sift_scales} verdeutlicht die Vorgehensweise an einem Beispiel mit $S=5$ Skalen pro Oktave, sowie insgesamt $O=3$ Oktaven.
