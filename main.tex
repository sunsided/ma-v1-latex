...
\cite{lowe1999recognition,lowe2004distinctive} sind die Original-Paper von Lowe zu \algo{SIFT}. Er merkt an, dass verschiedene Detektoren wie z.B. der Harris-Kantendetektor stark auf Bildskalierungen reagieren und demnach ungeeignet sind.
...
% \url{http://www.nowozin.net/sebastian/tu-berlin-2006/libsift/} Sebastian Nomozin macht es in der Beschreibung seines Codes falsch
...
Konkret bedeutet \cref{eq:convolution_twosigma_sqrt}, dass die wiederholte Anwendung desselben Gaußoperators auf ein Bild identisch ist mit der direkten Anwendung eines mit $\sigma' = \sqrt{2}\sigma$ "`breiteren"' Operators auf das Ursprungsbild, wobei sich das exponentielle Wachstum in $k^p\sigma_1$ als negativ herausstellt.

Der Einfluss von Werten außerhalb $\mu \pm 3\sigma$ kann für die Betrachtung des Operators vernachlässigt werden (vgl. \cite[176]{sonka2008image}); um jedoch einen zu starken Beschnitt der Werte in den Randbereichen des Kernes zu vermeiden, wählt etwa \cite{Nguyen:2007:GG:1407436} eine entsprechende Filterbreite von $\floor*{3\sigma}$ je Dimension%
\footnote{Für das Ermitteln einer ungeraden Filtergröße kann der Wert $2\floor*{\frac{3\sigma}{2}}+1$ verwendet werden.}, \cite{lowe1999recognition} wählt einen Kern der Breite $7$ (d.h. einen Radius von drei Werten in jede Richtung).
Diese Abhängigkeit der Filtergröße von seiner Parametrierung führt nun trotz Ausnutzung der Separierbarkeit im vorliegenden Fall zu einem ebenfalls exponential steigenden Rechenaufwand.

Abhilfe schafft es, anstatt der Filterung
%
\begin{align}
\mat H_p &= \mat G_{Gauss}(k^p \, \sigma) \ast \mat I
\end{align}
%
rekursiv vorzugehen, wobei analog \cref{eq:convolution_twosigma_sqrt} die folgende Annahme aufgestellt wird:
%
\begin{align}
\sigma_{p+1} &= \sqrt{ (\sigma' )^2 + ( \sigma_{p} )^2 } \label{eq:sift_convolution_twosigma_sqrt} 
\end{align}
%
Über die Identität
%
\begin{align}
\begin{split}
\sigma_{p+1} &= k^p \, \sigma_1 \\ 
			 &= k \left( k^{p-1} \sigma_1 \right) \\		 
			 &= k \, \sigma_p \\
\end{split}
\end{align}
%
...
%
\begin{align}
k \sigma_1 &= \sqrt{ (\sigma' )^2 + ( \sigma_1 )^2 } \\
% k^2 \sigma^2_p &= (\sigma')^2 + \sigma^2_1 \\
% \sigma^2_1 ( k^2 - 1) &= (\sigma')^2 \\
\sigma' &= \sigma_1 \sqrt{ k^2 - 1 }
\end{align}
%
wodurch es möglich wird, die Bilder mit einem deterministischen Kern fester Größe rekursiv zu filtern, so dass
%
\begin{align}
\mat H_{p+1} &= \mat G_{Gauss}(\sigma') \ast \mat H_p
\end{align}
