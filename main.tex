...
%
\begin{align}
e_1(\mat I) &= |\frac{\partial}{\partial x} \mat I| + |\frac{\partial}{\partial y} \mat I| \label{eq:seamEnergy1}
\end{align}
%
...
\end{align}
%
der sich bei einer diskreten Schrittweite von $\Delta x = \Delta k = 1$ als Faltung (\engl{convolution}) $(f \ast g)[k]$ von $f[k]$ mit der Funktion
\begin{align}
g[k] = 
	\begin{cases}
...
Hierbei wird die relative Veränderung eines Wertes zu seinen Nachbarwerten beschränkt, was die Amplitude des Rauschsignales auf Kosten der Amplitude des Nutzsignales mindert.
Eine zugrunde liegende Annahme dürfte sein, dass sich natürlich vorkommende -- genereller: energetisch günstige -- Folgezustände nicht stark voneinander unterscheiden%
...
Einen einfachen, aber effektiven Glättungsprozess stellt dabei der (lokal) gewichtete Mittelwert dar, die die Intensität eines Wertes -- z.B. Pixels -- aus seiner Nachbarschaft ermittelt.
Exemplarische, eindimensionale Glättungskern in horizontaler Richtung stellen \cref{eq:len3meanKernel1D} (ein einfacher Mittelwert über die direkte Nachbarschaft), sowie \cref{eq:len3smoothKernel1D} (ein gewichteter Mittelwert über dieselbe Nachbarschaft) dar:
%
\begin{align}
\mat G_{mean} = \begin{bmatrix}
 1 & 1 & 1
 \end{bmatrix} \label{eq:len3meanKernel1D} \\
\mat G_{smooth} = \begin{bmatrix}
 1 & 2 & 1
 \end{bmatrix} \label{eq:len3smoothKernel1D}
\end{align}

In beiden Fällen kollidiert die Aufgabe der Differenzierung in Laufrichtung des Filters mit der Glättung in derselben Richtung, da beide Operationen gegenteilige Ziele verfolgen (Detektion, sowie Minderung von Sprüngen).

Der \algo{Sobel-Operator} (nach \cite{Sobel:1968:Talk}, beschrieben in \cite{Sobel:2015:Online}) löst dieses Problem im zweidimensionalen Raum, indem er die Glättungsoperation orthogonal zur Suchrichtung ausführt: Soll in horizontaler Richtung differenziert werden, wird hierzu ("`zuerst"') in vertikaler Richtung geglättet.
Der zweidimensionale Filterkern des Sobel-Operators zur Differenzierung in horizontaler Richtung $\mat G_{x}$ ergibt sich folglich aus der Überlagerung der Kerne $\mat G_{smooth}$ und $\mat G_{diff}$ als
%
\begin{align}
\mat G_{x} &= 
\begin{bmatrix}
 ...
 2 & 0 & -2 \\
 1 & 0 & -1
 \end{bmatrix} = \mat G_{smooth}^T \cdot \mat G_{diff} \label{eq:sobel3x}
\end{align}
...
\begin{bmatrix}
 ...
 0 & 0 & 0 \\
 -1 & -2 & -1
 \end{bmatrix} = \mat G_{diff}^T \cdot \mat G_{smooth} \label{eq:sobel3x}
\end{align}

Eine nennenswerte Eigenschaft der Differenzierung von Bildern ist, dass die so ermittelten Intensitätssprünge oft\todo{wie oft ist oft} einhergehen mit dem Vorhandensein von Kanten der abgebildeten Objekte, weswegen der Sobel-Operator als einfacher%
\footnote{Der Sobel-Operator ist nicht anisotrop; seine Sensitivität gegenüber diagonalen Kanten -- insbesonderer solcher im 45°-Winkel zu den beiden Suchrichtungen -- ist minimal.}, wenngleich effizienter Kantendetektor eingesetzt werden wird.

Analog \cref{eq:seamEnergy1} ergibt sich folglich das Energiebild
%
\begin{align}
e_1(\mat I) &= |\mat G_x \ast \mat I| + |\mat G_y \ast \mat I| \label{eq:sobelEnergy}
\end{align}
%
als absolute Summe des mit jedem Sobel-Operatoren separat gefalteten Bildes.
